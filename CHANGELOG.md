...
**真正的解决方案**：不是「一劳永逸地预编译 .syso」，而是设计了一套策略：
1. **每次编译前先去查 goversioninfo 的最新版本号**
2. **如果有新版本 → 重新编译 .syso**
3. **没有新版本 → 直接用已有的 .syso**
这样既避免了每次都装工具浪费编译时间，又保证了不被 stale 版本坑。

**具体做法**：`resource.syso` 预提交到仓库，CNB 流水线只需 `go build`，不装任何工具。但本地发版前运行 `tools/goversioninfo` 检查更新，有新版就重编 .syso 一并提交。

**感想**：一个杀软误报，从 v0.3.18 到 v0.3.35，跨了十几个版本才彻底解决。最后靠的不是"找到一个配置关掉它"，而是建立了一套版本检查机制——**不是更聪明，是更小心了。**
